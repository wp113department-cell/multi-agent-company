"""feature_flag_agent — LangGraph agent."""
...
_SUBMIT = {"name": "submit_feature_flag_agent", "description": "Submit feature_flag_agent result.", "input_schema": {"type": "object", "properties": {"summary": {"type": "string"}, "findings": {"type": "array", "items": {"type": "string"}}, "recommendations": {"type": "array", "items": {"type": "string"}}}, "required": ["summary"]}}
...
def make_feature_flag_agent_handlers(repo_path: str) -> dict[str, Any]:
    ...
    base["submit_feature_flag_agent"] = submit_h; base["_result"] = result
    return base

def run_feature_flag_agent(task_id: int, description: str, repo_path: str | None = None, on_heartbeat: Any = None, on_tool_call: Any = None) -> AgentResult:
    settings = get_settings(); repo = repo_path or str(settings.target_repo_path)
    handlers = make_feature_flag_agent_handlers(repo); result = handlers["_result"]
    msg = f"Task #{task_id} — {description}\n\nUse available tools to complete this task. Call submit_feature_flag_agent with your findings and summary when done."
    final_state = run_agent_graph(role_name="feature_flag_agent", model=settings.model_coder, tools=_TOOLS, tool_handlers=handlers, verification_cfg=_CFG, initial_message=msg, max_turns=20)
