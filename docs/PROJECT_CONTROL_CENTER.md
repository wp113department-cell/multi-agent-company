...
Last updated: 2026-07-22 (Gap-Closure Days 0-18 full re-audit + Day 19 production-readiness prep — deployment itself skipped)
...
| **Gap-Closure — Days 0-18 full re-audit** | ✅ Complete 2026-07-22 | Systemic `task_id`/`trace_id` bug fixed at 4 sites in `run_agent_graph()` (every TaskStarted/TaskCompleted/TaskFailed event and `agent_registry.current_task_id` had recorded the wrong id since this code was written); `HealthUpdated` now fires on the error path too; `fleet_checkpoint.save_checkpoint()` given its first real callers (7th "built but never called" instance); remaining `trace_id=""` sites in manager.py/api/agents.py fixed; `ensure_all_agents_registered()` added so `capability_registry` holds all 72 agents from process start instead of ~6 |
| **Day 19 prep — Production Readiness** | ✅ Prep complete 2026-07-22 (deployment itself skipped, by explicit user request) | `vercel.json` fixed (was `npm ci` with no lockfile in `apps/web/` — would fail on a real build; now `pnpm`, matching CI). `backend/.env.example` now documents all 93 real `Settings` fields (was missing 18). Stale TS-era root `.env.example` replaced with an accurate pointer; `apps/web/.env.example` added. New `docs/DEPLOYMENT.md` — Supabase/Railway/Render/Vercel steps for the user to run themselves. CI workflow and `Procfile` confirmed already correct, no changes needed |
...
| Days 0-18 gap-closure (task_id/trace_id bug, HealthUpdated on error, checkpoint wiring, remaining trace_id="" sites, agent registry eager-import bootstrap) | ✅ CLOSED | Gap-closure — 2026-07-22 |
| Day 19 production-readiness prep (vercel.json pnpm fix, .env.example completeness, DEPLOYMENT.md) | ✅ CLOSED (deployment itself intentionally skipped) | Day 19 prep — 2026-07-22 |
...
| **Gap-Closure — Days 0-18 Full Re-Audit + Day 19 Prep** | **2026-07-22** | **2707/2707** | Deepest gap-closure yet — covers Day 0 forward, not just recent days. Found a systemic bug present since `run_agent_graph()` was first written: 4 sites used the per-RUN `tid` trace correlation id where the real `task_id` was needed (`agent_registry.start_task`, TaskStarted/TaskCompleted/TaskFailed events) — every one of these had recorded the wrong id for every real run ever made. Also closed Gap 7's error-path `HealthUpdated` gap, gave `fleet_checkpoint.save_checkpoint()` its first real callers (7th "built but never called" instance), closed remaining `trace_id=""` sites, and found most of the 72 agents were only ever imported lazily (`capability_registry` held as few as ~6/72 for most of a process's life) — fixed with a new `ensure_all_agents_registered()` startup bootstrap. Day 19 prep (deployment itself skipped): fixed `vercel.json`'s `npm ci` (no lockfile in `apps/web/`, would fail on a real build) to pnpm; `backend/.env.example` now documents all 93 real fields (was missing 18); replaced stale TS-era root `.env.example`; added `apps/web/.env.example` and `docs/DEPLOYMENT.md`. CI workflow and `Procfile` confirmed already correct. Full report: `docs/reports/GAP_CLOSURE_DAY0_18_REPORT.md` |
